    ...
    <nav class="-mb-px flex flex-wrap gap-2 sm:gap-4" aria-label="Tabs Keuangan">
        ...
        @can('view saldo')
            <a href="{{ route('admin.keuangan.saldo') }}"
                class="inline-flex items-center border-b-2 px-1 py-1 text-sm font-medium transition {{ request()->routeIs('admin.keuangan.saldo') ? 'border-black text-black' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700' }}">
                Neraca Saldo
            </a>
        @endcan
    </nav>
